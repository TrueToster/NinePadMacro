...
#define ID_STYLE_TRANS_BLACK 1003
#define ID_STYLE_WHITE 1004
#define ID_STYLE_TRANS_WHITE 1005

NOTIFYICONDATA nid = {};
HWND hwnd = NULL;
bool g_Running = true;
int g_currentTheme = false;
...
void SaveSettings() {
    ...
    if (RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\NinePad", 0, NULL, 0, KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
        DWORD val = g_currentTheme;
        RegSetValueExA(hKey, "Theme", 0, REG_DWORD, (BYTE*)&val, sizeof(val));
        RegCloseKey(hKey);
    }
}
...
void LoadSettings() {
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\NinePad", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        if (RegQueryValueExA(hKey, "Theme", NULL, NULL, (BYTE*)&val, &size) == ERROR_SUCCESS) {
            g_currentTheme = val;
        }
        RegCloseKey(hKey);
    }
...
// transparent background, black nums
HICON CreateNumberIconTransBlack(int number) {
    HDC hdc = GetDC(NULL);
    HDC hdcMem = CreateCompatibleDC(hdc);
...
// white background
HICON CreateNumberIconWhite(int number) {
    HDC hdc = GetDC(NULL);
    HDC hdcMem = CreateCompatibleDC(hdc);
    HBITMAP hbm = CreateCompatibleBitmap(hdc, 16, 16);
    SelectObject(hdcMem, hbm);
...
// transparent background, white numbers
HICON CreateNumberIconTransWhite(int number) {
    HDC hdc = GetDC(NULL);
    HDC hdcMem = CreateCompatibleDC(hdc);
...
HICON CreateNumberIcon(int number) {
    switch(g_currentTheme) {
        case 0: return CreateNumberIconBlack(number);
        case 1: return CreateNumberIconTransBlack(number);
        case 2: return CreateNumberIconWhite(number);
        case 3: return CreateNumberIconTransWhite(number);
        default: return CreateNumberIconBlack(number);
    }
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    if (message == WM_TRAYICON) {
        if (lParam == WM_RBUTTONUP) {
            ...
            AppendMenuW(hMenu, MF_STRING | (g_currentTheme == 0 ? MF_CHECKED : MF_UNCHECKED), ID_STYLE_BLACK, L"Темная тема");
            AppendMenuW(hMenu, MF_STRING | (g_currentTheme == 1 ? MF_CHECKED : MF_UNCHECKED), ID_STYLE_TRANS_BLACK, L"Прозрачная тема");
            AppendMenuW(hMenu, MF_STRING | (g_currentTheme == 2 ? MF_CHECKED : MF_UNCHECKED), ID_STYLE_WHITE, L"Светлая тема");
            AppendMenuW(hMenu, MF_STRING | (g_currentTheme == 3 ? MF_CHECKED : MF_UNCHECKED), ID_STYLE_TRANS_WHITE, L"Прозрачная светлая тема");
    ...
    if (message == WM_COMMAND) {
        ...
        if (id == ID_STYLE_BLACK) g_currentTheme = 0;
        if (id == ID_STYLE_TRANS_BLACK) g_currentTheme = 1;
        if (id == ID_STYLE_WHITE) g_currentTheme = 2;
        if (id == ID_STYLE_TRANS_WHITE) g_currentTheme = 3;
        
        if (id >= ID_STYLE_BLACK || id <= ID_STYLE_TRANS_WHITE) {
            SaveSettings();
